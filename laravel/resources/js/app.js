class App {
  constructor() {
    ...
    this.baseUrl = '';  // Relative URLs - same origin
  ...
  async upload(file) {
    ...
    try {
      ...
      const uploadRes = await fetch('/sessions', {
        method: 'POST',
        headers,
        body: formData
      });
  ...
  async startProcessing() {
    ...
    try {
      ...
      const res = await fetch(`/sessions/${this.sessionId}/process`, {
        method: 'POST',
        headers,
  ...
  async fetchResults() {
    try {
      const res = await fetch(`/sessions/${this.sessionId}/results`);
