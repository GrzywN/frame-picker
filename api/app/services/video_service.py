...
class VideoService:
    ...
    def validate_video_file(self, video: UploadFile) -> Dict[str, Any]:
        ...
        if video.size and video.size > settings.MAX_FILE_SIZE:
            validation["valid"] = False
            validation["errors"].append(f"File size exceeds {settings.MAX_FILE_SIZE / (1024*1024):.0f}MB limit")
        ...
        if video.filename:
            ...
            if ext not in allowed_extensions:
                validation["valid"] = False
                validation["errors"].append(f"File type {ext} not supported. Allowed: {', '.join(allowed_extensions)}")
